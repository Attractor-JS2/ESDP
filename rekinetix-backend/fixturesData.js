const redFlagTypesData = [
  {
    title:
      'Гипертоническая болезнь, ИБС, стенокардия напряжения, кардиостимулятор, операции по замене клапанов сердца',
    orderNumber: 1,
  },
  {
    title: 'Онкологические заболевания',
    orderNumber: 2,
  },
  {
    title: 'Наличие доброкачественных образований в органах',
    orderNumber: 3,
  },
  {
    title: 'Беременность',
    orderNumber: 4,
  },
  {
    title: 'Сахарный диабет',
    orderNumber: 5,
  },
  {
    title: 'Асцит',
    orderNumber: 6,
  },
  {
    title: 'Аневризмы',
    orderNumber: 7,
  },
  {
    title: 'Мочекаменная и желчекаменная болезни',
    orderNumber: 8,
  },
  {
    title: 'Остеомиелит',
    orderNumber: 9,
  },
  {
    title: 'Остеопороз',
    orderNumber: 10,
  },
  {
    title: 'Перелом позвоночника и костей',
    orderNumber: 11,
  },
  {
    title: 'Синдром конского хвоста',
    orderNumber: 12,
  },
  {
    title: 'Свежие грыжи межпозвонковых дисков',
    orderNumber: 13,
  },
  {
    title: 'Психические заболевания',
    orderNumber: 14,
  },
  {
    title: 'Варикозная болезнь',
    orderNumber: 15,
  },
];

const stageTypesData = [
  {
    title: '1. Обезболивание',
    stageNumber: 1,
  },
  {
    title: '2. Мобилизация',
    stageNumber: 2,
  },
  {
    title: '3. НМА и Стабилизация',
    stageNumber: 3,
  },
  {
    title: '4. Восстановление фукнций миофасциальных лент',
    stageNumber: 4,
  },
  {
    title: '5. Профилактика',
    stageNumber: 5,
  },
];

const mockData = {
  redFlagTypesData,
  stageTypesData,
};

module.exports = mockData;
